...
export function generateCalculatorMetadata(key: string): Metadata {
  const cfg = calculatorsConfig[key]
  if (!cfg) return { title: 'Calculadora não encontrada' }
  return {
    title: cfg.title,
    description: cfg.description,
    keywords: cfg.keywords,
    alternates: { canonical: `${siteUrl}${cfg.path}` },
    openGraph: {
      title: cfg.title,
      description: cfg.description,
      ...
      locale: 'pt_BR',
      type: 'website',
      images: [{ url: `${siteUrl}/og-image.png`, width: 1200, height: 630, alt: cfg.title }],
    },
    twitter: {
